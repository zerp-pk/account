...
import { EditRevenueCategoriesProps, RevenueCategoriesFormData } from './types';

export default function Edit({ revenuecategories, onSuccess, chartofaccounts }: EditRevenueCategoriesProps) {
    const { t } = useTranslation();
    const { data, setData, put, processing, errors } = useForm<RevenueCategoriesFormData>({
        category_name: revenuecategories.category_name ?? '',
        category_code: revenuecategories.category_code ?? '',
        gl_account_id: revenuecategories.gl_account_id?.toString() || '',
        description: revenuecategories.description ?? '',
        is_active: revenuecategories.is_active ?? true,
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('account.revenue-categories.update', revenuecategories.id), {
            onSuccess: () => {
                onSuccess();
            }
        });
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{t('Edit Revenue Categories')}</DialogTitle>
            </DialogHeader>
